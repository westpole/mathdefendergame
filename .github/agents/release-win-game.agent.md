...
4. Verify the current branch is `master` before running any release command.
...
8. After checks and build pass, run exactly one release command matching the requested version bump: `npm run release:patch`, `npm run release:minor`, or `npm run release:major`.
9. Do not edit source files manually unless the user explicitly asks for release-related code changes. This agent is for executing and verifying the release workflow.
...
4. Run the prebuild checks in order.
5. Run `npm run build:win` to generate the production build.
6. Run the matching release command so `npm version` creates the git tag and the repo scripts generate the changelog, commit, and push.
7. Report the version bump used, checks run, build result, and whether commit, tag, and push completed.
